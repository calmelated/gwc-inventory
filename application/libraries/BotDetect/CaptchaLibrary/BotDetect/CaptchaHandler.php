...
if (!array_key_exists('get', $_GET) || !StringHelper::HasValue($_GET['get'])) {
  HttpHelper::BadRequest('command');
}
$commandString = StringHelper::Normalize($_GET['get']);

// GbPlugin workaround, see http://captcha.biz/doc/asp/known-issues.html#gbplugin
if (array_key_exists('HTTP_USER_AGENT', $_SERVER) &&
    false !== strpos($_SERVER['HTTP_USER_AGENT'], 'GbPlugin')) {
  HttpHelper::IgnoreRequest();
}
...
function GetNormalSound() {
  ...
  try {
    ...
    if (!array_key_exists('d', $_GET)) { // javascript player not used, we send the file directly as a download
      $downloadId = CryptoHelper::GenerateGuid();
...
function Get_iOS_ChunkedSound() {
  ...
  $isJavaScriptPlayerRequest = (array_key_exists('d', $_GET) && StringHelper::HasValue($_GET['d']));
  if ($isJavaScriptPlayerRequest) {
    // when javascript is enabled, we can detect the first request because the timestamp changed
    $soundClickId = StringHelper::Normalize($_GET['d']);
  ...
  if ($rangeStart == $rangeEnd || $rangeEnd > $totalSize) {
    Clear_iOS_SoundData();
    HttpHelper::BadRequest('invalid byte range');
  }
  ...
  try {
    ...
    //$mimeType = $captcha->SoundMimeType; // TODO
...
function Detect_iOS_ChunkedRequest() {
  ...
  if (array_key_exists('HTTP_X_PLAYBACK_SESSION_ID', $_SERVER) && 
      StringHelper::HasValue($_SERVER['HTTP_X_PLAYBACK_SESSION_ID']) &&
      array_key_exists('HTTP_RANGE', $_SERVER) && 
      StringHelper::HasValue($_SERVER['HTTP_RANGE'])) {
    $detected = true;
  }
  return $detected;
}
...
function GetSoundByteRange() {
  ...
  if (!StringHelper::HasValue($rangeStr)) {
    return;
  }
...
function GetCacheFilename() {
  return ServerHelper::CombinePaths(sys_get_temp_dir(), 'cached_iOS_SoundData' . session_id());
}
